...
#Region "Окно ввода серийного номера платы"
        ...
        If e.KeyCode = Keys.Enter And SerialTextBox.TextLength = GetLenSN(LOTInfo(8)) Then
...
#Region "ПОСЛЕДОВАТЕЛЬНОСТЬ ОБРАБОТКИ СЕРИЙНОГО НОМЕРА"
        ...
        End Sub
#End Region
#Region "Проверка регистрации платы на FasSnReg "
...
#Region "Функция определения результата этапа"
    ...
    Private Function GetStepResult() As ArrayList
        ...
        PCBStepRes = SelectListString($"Use FAS
                select 
                tt.StepID,tt.TestResultID, tt.StepDate ,tt.SNID,tt.PCBID
                from  (SELECT *, ROW_NUMBER() over(partition by pcbid order by stepdate desc) num 
                FROM [FAS].[dbo].[Ct_OperLog] 
                where LOTID = {LOTID} and  SNID  ={PCBCheckRes(1)}) tt
                where  tt.num = 1")
